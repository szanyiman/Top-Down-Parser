...
void TopDownParser::ParsingState::print_instance_to_std_err(){
    ...
    //std::cerr<<"symbol alternative: "<<symbol_alternative<<std::endl;
    //std::cerr << "input_pointer: "<< input_pointer << std::endl << std::endl;
...
void TopDownParser::extend(std::string non_terminal, unsigned int num_of_alternative){
    ...
    new_state->level++;
    new_state->symbol_alternative;

    list_of_states.push_back(new_state);
    std::cerr<<non_terminal<<" was extended to the "<<num_of_alternative+1<<" alternative"<<std::endl;
...
void TopDownParser::match_input(){
    ...
    if (list_of_states[list_of_states.size() - 1]->beta == "b")
    {
        char word = 'b';
        if (list_of_states[list_of_states.size() - 1]->beta[0] == word) {
            std::cerr << std::endl << "Successful matching" << std::endl;
            successful_matching();
    }
    }
...
void TopDownParser::successful_matching(){
    ...
    ps->i++;
    input_pointer++;
    if (input_pointer==input_word.length()){
...
void TopDownParser::unsuccessful_matching(){
    std::cerr<< std::endl << "The matching was unsuccessful >> backtrack in the input"<<std::endl;
    list_of_states[list_of_states.size()-1]->print_instance_to_std_err();
    backtrack_in_input();
}
